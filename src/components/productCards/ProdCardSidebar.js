...
function ProdCardSidebar({ product: { id, title, image, price, rating: { rate }}, count }) {
    ...
    return (
        <div className="bg-purple-200 hover:bg-purple-200 transform transition hover:scale-102 mx-2 my-3 px-3 py-1 flex flex-wrap flex-row rounded-md">
            ...
            <div className="flex w-full py-1">
                <button className="bg-purple-400 transition transform hover:scale-110 w-1/4 py-0 px-2 font-semibold rounded-md" onClick={() =>{
                ...
                <div className="flex space-x-3 justify-around flex-grow px-8 space-x-4">
                    ...
                    <button onClick={() => setCartProducts({...cartProducts, [id]: {...cartProducts[id],count: Math.max(0,cartProducts[id].count-1)}})} className="bg-purple-400 transition transform hover:scale-110 px-2 py-0 rounded-full" ><i className="fa fa-minus font-bold text-xl"></i></button>
...
export default ProdCardSidebar;